...
# ---------- Quantile functions for each method (all weighted correctly) ----------
def get_quantile(method, cid, b):
    ...
    if method == "global_lac":
        ...
        if len(bin_cal) >= MIN_CALIB_PER_BIN:
            ...
            return conformal_qhat(cal["score_correct"])   # fallback to overall client quantile
...
for key, label in method_labels.items():
    r = results[key]
    rows.append([label, r["Coverage"], r["Useful Rate"], r["Hallucination"], r["Set Size"], r["Precision@Commit"]])
...
plt.savefig("final_weighted_table.pdf", dpi=200, bbox_inches="tight")
plt.savefig("final_weighted_table.png", dpi=200, bbox_inches="tight")
print("Saved: final_weighted_table.pdf / .png")

# Also print the numbers so you can check
